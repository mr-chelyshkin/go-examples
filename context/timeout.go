...
func workerTimeOut(ctx context.Context, workerNum int, out chan<- int) {
...
func main()  {
	...
	ctx, _ := context.WithTimeout(context.Background(), workTime) // set timeout (workTime value)
	result := make(chan int, 1)

	for i:=0;i<10;i++ {
		go workerTimeOut(ctx, i, result)
	}
